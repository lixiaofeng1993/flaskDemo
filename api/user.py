...
def get_user(username):
    """获取某个用户信息"""
    login_user = request.headers.get("username")
    redis_token = redis_db.handle_redis_token(login_user)  # 从redis中取token
    token = request.headers.get("token")
    if redis_token:
        if redis_token == token:
...
def user_register():
    ...
    telephone = str(request.json.get("telephone", "")).strip()  # 手机号
    ...
    if username and password and telephone:  # 注意if条件中 "" 也是空, 按False处理
        ...
        elif not (len(telephone) == 11 and re.match("^1[3,4,5,7,8]\d{9}$", telephone)):
...
@app.route("/delete/user/one/", methods=['POST'])
def user_delete_one():
    body = request.form.to_dict()
    username = body.get("username").strip()
    admin_user = request.headers.get("admin_user", "").strip()  # 当前登录的管理员用户
    token = request.headers.get("token", "").strip()  # token口令
    if admin_user and token:
        redis_token = redis_db.handle_redis_token(admin_user)  # 从redis中取token
        if redis_token:
            if redis_token == token:  # 如果从redis中取到的token不为空，且等于请求body中的token
